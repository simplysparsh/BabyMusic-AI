...
export default function EmailOnboardingModal({ isOpen, onComplete, userProfile, onShouldShowIOSInstallInstructions }: EmailOnboardingModalProps) {
  ...
  useEffect(() => {
    if (isOpen && userProfile) {
      console.log('EmailOnboardingModal opened for user profile:', userProfile);
  ...
  const babyName = userProfile.babyName || 'your baby';
  ...
  const handleInfoSubmit = async () => {
    ...
    try {
      ...
      console.log('Completing email onboarding with data:', profileUpdates);
      setIsUpdating(true);
      ...
      if (user && user.id) {
        ...
        console.error('User ID not available in EmailOnboardingModal for updateProfile');
        setError('User session error. Please try again.');
      }
      
    } catch (error) {
      console.error('Failed to update profile during email onboarding:', error);
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-white/60">Help us personalize the experience for {babyName}</p>
        </div>
      </div>
